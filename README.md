# Cross-platform SQLite storage plugin for Cordova/PhoneGap - cordova-sqlite-ext-common version branchh
...
- SQLite version `3.26.0` included with the following build settings:
  ...
  - `SQLITE_ENABLE_JSON1`
  - `SQLITE_ENABLE_RTREE`
  - `SQLITE_DEFAULT_PAGE_SIZE=1024` (all platforms Android/iOS/macOS/Windows) and `SQLITE_DEFAULT_CACHE_SIZE=2000` (Android & Windows only) to avoid "potentially distruptive change(s)" from SQLite 3.12.0 described at: <http://sqlite.org/pgszchng2016.html>
...
- _The following features are available in XXX [litehelpers / cordova-sqlite-ext](https://github.com/litehelpers/cordova-sqlite-ext):_
  - REGEXP (Android/iOS/macOS)
  - SELECT BLOB data in Base64 format (all platforms Android/iOS/macOS/Windows)
- _The following feature are available in XXX [litehelpers / cordova-sqlite-ext](https://github.com/litehelpers/cordova-sqlite-ext) missing in this plugin version branch:_
...
- SELECT BLOB column value type is not supported consistently across all platforms (not supported on Windows). It is recommended to use the built-in HEX function to SELECT BLOB column data in hexadecimal format, working consistently across all platforms. As an alternative: SELECT BLOB in Base64 format is supported by this plugin version, [`brodybits/cordova-sqlite-ext`](https://github.com/brodybits/cordova-sqlite-ext) (permissive license terms) and [litehelpers / Cordova-sqlite-evcore-extbuild-free](https://github.com/litehelpers/Cordova-sqlite-evcore-extbuild-free) (GPL or commercial license options).
